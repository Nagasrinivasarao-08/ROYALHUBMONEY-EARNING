...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, onLogout, isAdmin }) => {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-gray-900 flex flex-col font-sans">
        ...
        <div className="max-w-md mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-bold text-lg tracking-wide text-amber-500">
                {isAdmin ? 'Royal Admin' : 'Royal Invest'}
            </span>
          </div>
          <button 
            onClick={onLogout}
          ...
          >
            <span className="text-xs font-medium uppercase tracking-wider">{isAdmin ? 'Logout' : ''}</span>
            <LogOut size={20} />
          </button>
        </div>
      </header>
      ...
      {/* Main Content */}
      <main className={`flex-1 overflow-y-auto bg-[#f8f5f2] ${!isAdmin ? 'pb-24' : ''}`}>
        <div className="max-w-md mx-auto px-4 py-6">
          {children}
        </div>
      </main>

      {/* Bottom Navigation - HIDDEN FOR ADMINS ("Only Admin Panel") */}
      {!isAdmin && (
